...
#error "steamApi: Unsupported platform"
    ...
    SteamAPI_Init steamApiInit = (SteamAPI_Init)atheneumAddress(&self->atheneum, "SteamAPI_Init");
    ...
    self->runCallbacks = (SteamAPI_RunCallbacks)atheneumAddress(&self->atheneum, "SteamAPI_RunCallbacks");
    self->registerCallback = (SteamAPI_RegisterCallback)atheneumAddress(&self->atheneum, "SteamAPI_RegisterCallback");
    self->registerCallResult
        = (SteamAPI_RegisterCallResult)atheneumAddress(&self->atheneum, "SteamAPI_RegisterCallResult");

    self->createSteamUtilsInstance
        = (SteamAPI_SteamUtils_v010)atheneumAddress(&self->atheneum, "SteamAPI_SteamUtils_v010");

    self->setWarningMessageHook = (SteamAPI_ISteamUtils_SetWarningMessageHook)atheneumAddress(
